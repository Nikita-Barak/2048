...
public class Figure : MonoBehaviour
{
    ...
    [SerializeField] float explosionForce = 200f; // Max force applied to figures
    ...
    private void Move()
    {
        ...
        Vector2 newPosition = rb.position + new Vector2(xInput * moveSpeed * Time.deltaTime, 0);
    ...
    private int generateNum()
    {
        ...
        float[] weights = new float[RandomFigureSpawner.Instance.currentNumberPow];
        float totalWeight = 0;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (otherFigure != null && otherFigure.number == this.number)
        {
    ...
    private void AdjustScale()
    {
        ...
        float scaleMultiplier = Mathf.Log(number, 4) * growSpeed; 

        // Apply the new scale
        float newScale = baseScale + scaleMultiplier;
    ...
    public void TriggerExplosionPhysics()
    {
        ...
        Collider2D[] nearbyFigures = Physics2D.OverlapCircleAll(transform.position, explosionRadius);
        

        foreach (Collider2D col in nearbyFigures)
        {
            ...
            if (rb != null && rb != this.rb)
            {
                ...
                float forceMagnitude = explosionForce / rb.mass;
